...
export class ItemsEffects {
  ...
  loadItems$ = this.actions$.pipe(
    ...
    map(action => action.payload),
    switchMap(filters => this.persistence.getItems(filters).pipe(
      catchError(handleError(this.toast, 'Load All Failed', itemActions.loadAll.failed))
    )),
  ...
  loadItem$ = this.actions$.pipe(
    ...
    map(action => action.payload),
    switchMap(id => this.persistence.getItem(id).pipe(
      catchError(handleError(this.toast, 'Load Item Failed', itemActions.loadOne.failed))
    )),
    delay(DELAY_SIMULATION),
  ...
  addItem$ = this.actions$.pipe(
    ...
    map(action => action.payload),
    switchMap(item => this.persistence.saveItem(item).pipe(
      catchError(handleError(this.toast, 'Add Item Failed', itemActions.addOne.failed))
    )),
  ...
  saveItem$ = this.actions$.pipe(
    ...
    map(action => action.payload),
    switchMap(item => this.persistence.saveItem(item).pipe(
      catchError(handleError(this.toast, 'Save Item Failed', itemActions.saveOne.failed))
    )),
    delay(DELAY_SIMULATION),
  ...
  deleteItem$ = this.actions$.pipe(
    ...
    map(action => action.payload),
    switchMap(item => this.persistence.deleteItem(item).pipe(
      catchError(handleError(this.toast, 'Remove Item Failed', itemActions.removeOne.failed))
    )),
    delay(DELAY_SIMULATION),
